...
function setGraphRange(x) { graphRange = x; updatePopup(); }
function updatePopup() {
	if (popupPindex === null) return;
	$.getJSON("getpointdata?recid="+popupPindex.substr(4)+"&range="+graphRange, function(dataret) {
		...
		*/
		var hardmax = [data.hard_max];
		var hardmin = [data.hard_min];
		var softmax = [data.soft_max];
		var softmin = [data.soft_min];
		...
		hardmax.unshift(data.hard_max);
		hardmin.unshift(data.hard_min);
		softmax.unshift(data.soft_max);
		softmin.unshift(data.soft_min);
		
		points[pindex].times = dataret.histtime;
		points[pindex].values = dataret.histdata;
		points[pindex].histmaxv = dataret.histmaxv;
		points[pindex].histminv = dataret.histminv;
		points[pindex].limits = {hardmax:hardmax, hardmin:hardmin, softmax:softmax, softmin:softmin};
...
window.setTimeout(function(){ $('#popLoading').popup('reposition','positionTo: window'); },1);
});
</script>
...
while ($row = mysql_fetch_array($result)) {
	...
	if ($row['ip_address'] == "") {
		...
		$clines=explode("\r\n",$countent);
		if (isset($clines[2])) $c1 = explode(":",$clines[2]); else $c1="";
		if (isset($cllnes[3])) $c2 = explode(":",$clines[3]); else $c2="";
		if (count($c2) >= 2) $c2=trim($c2[1]); else $c2="";
		switch ($c2) {
	...
	echo "<span class='x-dval ui-li-count $cerr' id=\"dval".$row['disp_recid']."\" xdata-info=\"".$devinfo."\" title='dval".$row['disp_recid'].": ".$row['dis_functional_name']." - ".$row['description']." soft:".$row['soft_min_value']."-".$row['soft_max_value']."; hard:".$row['hard_min_value']."-".$row['hard_max_value']."\n".$row['alarm_name']."(".($row['alarm_type']==0?"individual":"aggregate").")'><span id='a'></span><span id='b'>$countent</span>";
...
<div data-role="popup" id="popLoading" style="min-width: 300px; min-height: 220px;" xxdata-transition="pop">
	<div data-role="header" data-theme="b" style="text-align:center;"><h1 style="margin: 0em;" id='loadingtext'>Loading...</h1></div>
	<div data-role="content" style="background-color: #eeeeee">
