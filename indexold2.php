...
.sparkline { background-color: #fff; 
...
*/
	...
	$(".sparkline").sparkline({defaultPixelsPerValue: 1});
	$(".sparkline").bind('sparklineClick', function(e) {
		console.log('clicked!');
		console.log(e);
		var pindex = $(e.target).attr('pointsindex');
		console.log(pindex);
		var desc = $(points[pindex]).siblings('.desc').html();
		$('#poptext').html(desc);
		$('#popGraph').popup('open');
		$('#popspark').sparkline(points[pindex].valarray, {height:'100px',width:'100px'}).css({marginLeft: 'auto', marginRight: 'auto'});
	});
//	$("#popGraph").popup();
}
...
function update_values(){
	var that = points[pointsindex];

	$.getJSON("getdata.php?id="+escape($(that).attr('id'))+"&info="+escape($(that).attr('xdata-info')), 
		function(data) {
			timeoutValue = 1;
			if (data.value != ' ') {
				var cell=$('#'+data.id+" #b");
				...
				if (!(that.valarray)) {
					that.valarray=[null,null,null,null,null,null,null,null,null,null,null,null,null,null,val];
					$('#'+data.id+" .sparkline").attr('pointsIndex',pointsindex);
				} else {
					that.valarray.push(val);
					while (that.valarray.length > 15) {
						that.valarray.shift();
					}
				}
				
				$('#'+data.id+" .sparkline").sparkline(that.valarray);
				
			}

			pointsindex++;
			if (pointsindex > points.length) pointsindex=0;
			window.setTimeout(update_values, timeoutValue); //+Math.random()*10);
		}
...
<div data-role="header" data-theme="b"><h1 style="margin: 0em;" id='poptext'>hi. this is test!! Please respond</h1></div>
<div data-role="content" style="background-color: #eeeeee">
<div id='popspark' style='background-color: #fff'></div>
</div>
</div>
</div> <!-- /ui-content -->
</div> <!-- /ui-page -->
